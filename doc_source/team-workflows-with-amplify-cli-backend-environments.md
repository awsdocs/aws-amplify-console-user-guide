...
A feature branch deployment consists of a **frontend** and [\(optionally\) a \*\*backend environment\*](https://docs.aws.amazon.com/amplify/latest/userguide/deploy-backend.html)\. The frontend is built and deployed to a global CDN, while the backend is deployed by the Amplify CLI to AWS\.
